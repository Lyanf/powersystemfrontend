        ...
        <el-main class="main">
          ...
          <el-dialog title="" :visible.sync="dialogFormVisible">
            <el-form :model="form">
              ...
              <el-form-item label="逻辑名称" :label-width="formLabelWidth">
                <el-input v-model="form.logic" autocomplete="off"></el-input>
              </el-form-item>
              ...
              <el-form-item label="类型" :label-width="formLabelWidth">
                <el-select v-model="form.type">
                  <el-option v-for="item in logicTypeList"
                             :value="item"
                             :key="item"
                  ></el-option>
                </el-select>
  ...
  export default {
    ...
    methods: {
      ...
      getTable: function () {
        ...
        axios.post('/api/getLogicType').then(function (response) {
          let newList = []
          for (var testt of response.data) {
            newList.push(testt['logictype'])
          }
          that.logicTypeList = newList
        })
        this.addLackNameFromid()
      },
      handleDelete: function (id) {
        let that = this
        axios.post('/api/deleteLogic', {id: id}).then(function (response) {
          that.getTable()
        })
      },
      ...
      sendForm: function (addFlag) {
        ...
        let that = this
        for(var index in this.conceptObj){
          if (this.form.concept === this.conceptObj[index]) {
            this.form.conceptid = index
            break
          }
        }
        if (addFlag === true) {
          axios.post('/api/addLogic', that.form).then(function (response) {
            that.getTable()
          })
        } else {
          axios.post('/api/changeLogic', that.form).then(function (response) {
            that.getTable()
          })
        }
      },
      addLackNameFromid:function () {
        ...
        axios.post('/api/getConcept').then(function (response) {
          that.allConceptFromBack = response.data
          for(var obj of that.allConceptFromBack){
            console.log(obj)
            that.conceptObj[obj.id] = obj.concept
          }
          for(var row of that.tableData){
            row['concept'] = that.conceptObj[row['conceptid']]
